...
func (o *Mysql) prepare() error {
	queries := map[string]map[string]string{
		"main": map[string]string{
			...
			"selectAllNodesDirty": "SELECT id, ip FROM nodes WHERE dirty = 1",
			...
			"updateNodeToDirty": "UPDATE nodes SET dirty = 1 WHERE id = ? AND dirty = 0",
			"updateNodeToMain": "UPDATE nodes SET dirty = 0 WHERE id = ? AND dirty = 1",
